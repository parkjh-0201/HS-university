...
client.subscribe("led", 1)           # "led" 토픽 구독 등록, QoS 1 설정
                                     # (메시지 최소 1회 수신 보장)
...
task = threading.Thread(target=send_thread)
                                     # send_thread 함수를 별도의 스레드로 생성

task.start()                         # 스레드 시작
                                     # 이 순간부터 발행과 구독이 동시에 실행됨

client.loop_forever()                # 메시지 수신을 위한 무한 대기 루프 실행
                                     # 프로그램이 종료되지 않고 계속 구독 유지
